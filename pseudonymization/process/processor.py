...
class Processor:
    ...
    def __init__(
        ...
        pseudo_tables_folder: str,
        sequencing_libraries: str,
        sequencing_libraries_sc: str,
        log_dir: str,
    ):
    ...
    def copy_libraries(self):
        ...
        shutil.copytree(
            self.sequencing_libraries_folder,
            self.sequencing_libraries_folder_sc,
            dirs_exist_ok=True,
        )
    ...
    def _mv_pseudonymizer_run_to_sc(self, run_name):
        shutil.move(
            os.path.join(self.sequencing_file_path, run_name),
            os.path.join(self.destination_folder, run_name),
        )

    def _initialize_based_on_record_type(
        self, full_run_path
    ) -> (FileRemover, RunPseudonymizer):
        logger = LoggingConfig.get_logger()
        if "SoftwareVersionsFile.csv" in os.listdir(
            full_run_path
        ) or "Alignment_1" in os.listdir(full_run_path):
            logger.info(f"Run {full_run_path} processed as NewMiseq")
            pseudonymizer = NewMiseqPseudonymizer(
                full_run_path, self.pseudonymization_tables_folder
            )
            ...
            pseudonymizer = OldMiseqPseudonymizer(
                full_run_path, self.pseudonymization_tables_folder
            )
