...
class NewMiseqPseudonymizer(OldMiseqPseudonymizer):
    ...
    def _try_pseudonimize_content_of_files(self, pred_number, pseudo_pred_number):
        subprocess.call(
            [
                "pseudonymization/helpers/replace_predictive_new_miseq.sh",
                self.run_path,
                pred_number,
                pseudo_pred_number,
            ]
    ...
    def _move_alignmant_data_from_unknown_folder(self):
        old_folder = os.path.join(
            self.run_path, "Alignment_1", self._find_folder_names_in_alignment()
        )
    ...
    def _find_folder_names_in_alignment(self) -> str:
        paths_in_alignment = [
            file
            for file in os.listdir(os.path.join(self.run_path, "Alignment_1"))
