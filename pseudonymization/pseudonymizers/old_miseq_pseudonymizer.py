...
from pseudonymization.pseudonimization_api.pseudonimize_patient import (
    PseudonymizePatient,
)
from pseudonymization.pseudonimization_api.pseudonimize_predictive import (
    PseudonymizePredictive,
)
...
class OldMiseqPseudonymizer(RunPseudonymizer):
    ...
    def __init__(self, run_path, pseudo_tables_folder_path):
        self.patient_pseudo_table = os.path.join(
            pseudo_tables_folder_path, "patients.json"
        )
        self.predictive_pseudo_table = os.path.join(
            pseudo_tables_folder_path, "predictive.json"
        )
        self.sample_pseudo_table = os.path.join(
            pseudo_tables_folder_path, "samples.json"
        )
    ...
    def pseudonymize(self):
        pred_pseudo_tuples = (
            self._get_all_predictive_numbers_pseudonymize_sample_sheet()
        )
        self.logger.debug(
            f"Predictive/pseudonym tuples generated: {pred_pseudo_tuples}"
        )
        ...
        for pred, pseudo in pred_pseudo_tuples:
            ...
            self.logger.info(
                f"Files and directories renamed for predictive number {pred} -> {pseudo}"
            )

            self._try_pseudonimize_content_of_files(
                pred, pseudo
            )  # This needs to run after the _pseudo_files_names_req
            self.logger.info(
                f"Contents of files renamed for predictive number {pred} -> {pseudo}"
            )

            clinical_data = ClinicalInfoFinder(self.run_path).collect_data(pred)

            if clinical_data:
                clinical_data_for_saving = self._prepare_clinical_data_for_saving(
                    clinical_data, pseudo
                )
                self._save_clinical_data(
                    clinical_data_for_saving,
                    os.path.join(self.run_path, "catalog_info_per_pred_number"),
                    pseudo,
                )

                self.logger.info(
                    f"Clinical data saved to {os.path.join(self.run_path, 'catalog_info_per_pred_number')}/{pseudo}"
                )

    def _get_all_predictive_numbers_pseudonymize_sample_sheet(
        self,
    ) -> list[tuple[str, str]]:
        ...
        pseudo_numbers = [
            predictive_pseudonimizer.pseudonymize(pred_number)
        ...
        predictive_pseudo_tuples = [
            (predictive_numbers[i], pseudo_numbers[i])
            for i in range(len(predictive_numbers))
        ]
    ...
    def _pseudonymize_files_with_pred_numbers(self, pred_number, pseudo_number):
        if "_" in pred_number:
            self._pseudonymize_file_names_recursively(
                pred_number, pseudo_number, self.run_path
            )
    ...
    def _pseudonymize_file_names_recursively(
        self, text_to_replace, replaced_text, current_file
    ):
        current_file_renamed = current_file[::-1].replace(
            text_to_replace[::-1], replaced_text[::-1], 1
        )[::-1]
        ...
        for file in os.listdir(current_file_renamed):
            ...
            if os.path.isdir(file_path):
                self._pseudonymize_file_names_recursively(
                    text_to_replace, replaced_text, file_path
                )
            else:
                os.rename(
                    os.path.join(current_file_renamed, file),
                    os.path.join(
                        current_file_renamed,
                        file.replace(text_to_replace, replaced_text),
                    ),
                )
    ...
    def _try_pseudonimize_content_of_files(self, pred_number, pseudo_pred_number):
        subprocess.call(
            [
                "pseudonymization/helpers/replace_predictive_old_miseq.sh",
                self.run_path,
                pred_number,
                pseudo_pred_number,
            ]
    ...
    def _prepare_clinical_data_for_saving(self, patient_clinical_data, pseudo_id):
        samples = [
            self._prepare_sample(sample, pseudo_id)
            for sample in patient_clinical_data["samples"]
        ]
        samples.sort()

        pseudo_patient_id = PseudonymizePatient(
            patient_clinical_data["ID"], self.patient_pseudo_table
        )()
        patient = Patient(
            pseudo_patient_id,
    ...
    def _save_clinical_data(
        self, patient: Patient, destination_path, pseudo_pred_number
    ) -> dict:
        serialized_patient = patient.serialize()
        if not os.path.exists(destination_path):
            os.mkdir(destination_path)
        with open(
            os.path.join(destination_path, f"{pseudo_pred_number}.json"), "w"
        ) as f:
