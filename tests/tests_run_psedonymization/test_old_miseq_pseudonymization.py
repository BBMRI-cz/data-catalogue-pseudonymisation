...
import os
import shutil
from pseudonymization.pseudonymizers.old_miseq_pseudonymizer import (
    OldMiseqPseudonymizer,
)
...
PSEUDONYMIZATION_PATIENTS_FILE = os.path.join(
    PSEUDONYMIZATION_FILES_FOLDER, "patients.json"
)
PSEUDONYMIZATION_SAMPLES_FILE = os.path.join(
    PSEUDONYMIZATION_FILES_FOLDER, "samples.json"
)
PSEUDONYMIZATION_PREDICTIVE_FILE = os.path.join(
    PSEUDONYMIZATION_FILES_FOLDER, "predictive.json"
)
...
def test_get_all_predictive_numbers_pseudonymize_sample_sheet(
    mocker, generate_list_of_pseudo_pred_numbers
):
    mocker.patch(
        "pseudonymization.pseudonymizers.old_miseq_pseudonymizer.PseudonymizePredictive.pseudonymize",
        side_effect=generate_list_of_pseudo_pred_numbers,
    )
    expected_predictive_numbers = [
        "102-20",
        "103-20",
        "104-20",
        "119-20",
    ...
    expected_pred_pseudo_numbers = [
        (pred, pseudo)
        for (pred, pseudo) in zip(
            expected_predictive_numbers, generate_list_of_pseudo_pred_numbers
        )
    ]

    predictive_pseudo_numbers = OldMiseqPseudonymizer(
        FAKE_RUN_FOR_TESTING, PSEUDONYMIZATION_FILES_FOLDER
...
def test_rename_files_recursively():
    pseudonimizer = OldMiseqPseudonymizer(
        FAKE_RUN_FOR_TESTING, PSEUDONYMIZATION_FILES_FOLDER
    )

    pseudonimizer._pseudonymize_file_names_recursively(
        "102-20", "000-00", FAKE_RUN_FOR_TESTING
    )
...
def test_rename_files_recursively_wrong_separator():
    ...
    pseudonimizer._pseudonymize_file_names_recursively(
        "152-20", "000-00", FAKE_RUN_FOR_TESTING
    )
...
def test_prepare_clinical_data_for_saving_non_empty(
    mocker,
    ...
    fake_OK_patient_pseudo_response = _generate_fake_OK_http_response(
        mocker, get_patient_psuedo_api_request
    )
    fake_OK_sample_pseudo_response = _generate_fake_OK_http_response(
        mocker, get_sample_pseudo_api_request
    )
    ...
    data_for_saving = pseudonimizer._prepare_clinical_data_for_saving(
        clinical_data_dict, random_pseudo_number
    )
    ...
    assert (
        data_for_saving.samples[0].sample_ID
        == "mmci_sample_12345678-1234-5678-1234-567812345621"
    )
...
def test_save_clinical_data(
    ...
    samples = [
        Tissue(
            get_tissue_dict_data,
            tissue_predictive_psuedo_id,
            PSEUDONYMIZATION_PREDICTIVE_FILE,
        ),
        Serum(
            get_serum_dict_data,
            serum_predictive_pseudo_id,
            PSEUDONYMIZATION_PREDICTIVE_FILE,
        ),
    ]

    pat = Patient(
        "mmci_patient_12345678-1234-5678-1234-567812345621",
        "Fri, 1 Mar 1990 00:00:00 GMT",
        1,
        samples,
    )

    folder_for_clinical_data = os.path.join(
        FAKE_RUN_FOR_TESTING, "catalog_info_per_pred_number"
    )
...
def generate_list_of_pseudo_pred_numbers():
    return [
        "mmci_predictive_12345678-1234-5678-1234-5678123456" + "{:02d}".format(number)
...
def get_serum_dict_data():
    serum_insides = {
        ...
        "biopsy_id": "2021/2344-1",
        "diagnosis": "C110",
        "material_type_id": "2",
        "type": "Serum",
        "patient_id": 111112,
        "predictive_id": "2020/1112",
        "sample_id": "BBM:2021:111:2",
        "samples_no": 3,
        "taking_date": "Fri, 1 Mar 2020 00:00:00 GMT",
    }
...
def get_tissue_dict_data():
    tissue_insides = {
        ...
        "available_samples_no": 2,
        "biopsy_id": "2021/2344-1",
        "cut_time": "Fri, 5 Mar 2021 00:00:00 GMT",
        "diagnosis": "C110",
        "freeze_time": "Fri, 5 Mar 2021 11:45:00 GMT",
        "material_type_id": "2",
        "morphology": "8720/36",
        "patient_id": 111112,
        "predictive_id": "2020/1112",
