...
import os
import shutil
from pseudonymization.pseudonymizers.new_miseq_pseudonymizer import (
    NewMiseqPseudonymizer,
)
...
def test_move_files_inside_alignment(mocker, uuid_generated_values):
    mocker.patch(
        "pseudonymization.pseudonymizers.new_miseq_pseudonymizer.subprocess.call",
        side_effects=mocker.Mock(),
    )
    mocker.patch(
        "pseudonymization.pseudonymizers.new_miseq_pseudonymizer.ClinicalInfoFinder.collect_data",
        return_value=[],
    )
    mocker.patch(
    ...
    assert os.path.exists(os.path.join(FAKE_RUN_FOR_TESTING, "Alignment_1"))
...
def test_clinical_info_folder_created(mocker, uuid_generated_values, get_test_patient):
    ...
    mocker.patch(
        "pseudonymization.pseudonymizers.old_miseq_pseudonymizer.PseudonymizePatient.__call__",
        side_effect=uuid_generated_values,
    )
    mocker.patch(
        "pseudonymization.pseudonymizers.new_miseq_pseudonymizer.ClinicalInfoFinder.collect_data",
