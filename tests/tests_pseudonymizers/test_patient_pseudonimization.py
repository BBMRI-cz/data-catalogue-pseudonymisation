...
def mock_POST_patient_OK_return():
    return {
        "data": {
            "patient_ID": "654321",
            "patient_pseudo_ID": "mmci_patient_12345678-1234-5678-1234-567812345678",
        },
        "isError": False,
        "message": "Success",
        "status_code": 200,
...
def test_patient_exist(mocker, mock_existing_patient):
    ...
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_patient.requests.get",
        return_value=fake_resp,
    )

    # Check if correct pseudo number was selected
    pat_pseudo = PseudonymizePatient(
        "123456", os.path.join(CURRENT_DIRECTORY, "patients.json")
    )
    pseudo_predictive_number = pat_pseudo()

    assert (
        pseudo_predictive_number == "mmci_patient_28237dcc-6c4b-4223-87e7-d99553f45a85"
    )
    ...
    for patient in data["patients"]:
        if patient["patient_ID"] == "123456":
            assert (
                ...
                == "mmci_patient_28237dcc-6c4b-4223-87e7-d99553f45a85"
            )


def test_patient_not_exist(
    mocker, mock_non_existing_patient, mock_new_uuid_value, mock_POST_patient_OK_return
):
    ...
    fake_POST_resp = _generate_fake_OK_http_response(
        mocker, mock_POST_patient_OK_return
    )
    ...
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_patient.requests.post",
        return_value=fake_POST_resp,
    )
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_patient.uuid.uuid4",
        return_value=mock_new_uuid_value,
    )

    pat_pseudo = PseudonymizePatient(
        "654321", os.path.join(CURRENT_DIRECTORY, "patients.json")
    )
    ...
    for patient in data["patients"]:
        if patient["patient_ID"] == "654321":
            assert (
                patient["patient_pseudo_ID"]
                == "mmci_patient_12345678-1234-5678-1234-567812345678"
            )
