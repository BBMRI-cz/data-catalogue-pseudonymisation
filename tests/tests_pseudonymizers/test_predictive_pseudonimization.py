...
PSEUDONYMIZATION_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "predictive.json"
)
...
def mock_existing_predictive():
    return {
        "predictive_ID": "2023-1111",
        "predictive_ID_unified": "1111-23",
        "predictive_pseudo_ID": "mmci_predictive_11f6afb1-9f11-4cee-ee47-eee5f5a8e0ee",
    }
...
def test_predictive_exist(mocker, mock_existing_predictive):
    ...
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_predictive.requests.get",
        return_value=fake_resp,
    )

    # Check if correct pseudo number was selected
    pat_pseudo = PseudonymizePredictive(PSEUDONYMIZATION_FILE)
    pseudo_predictive_number = pat_pseudo.pseudonymize("2023-1111")

    assert (
        pseudo_predictive_number
        == "mmci_predictive_11f6afb1-9f11-4cee-ee47-eee5f5a8e0ee"
    )
    ...
    for predictive in data["predictive"]:
        if predictive["predictive_number"] == "2023-1111":
            assert (
                predictive["pseudo_number"]
                == "mmci_predictive_11f6afb1-9f11-4cee-ee47-eee5f5a8e0ee"
            )


def test_predictive_not_exist(
    mocker,
    mock_non_existing_predictive,
    mock_new_uuid_value,
    mock_POST_predictive_OK_return,
):
    fake_GET_resp = _generate_fake_OK_http_response(
        mocker, mock_non_existing_predictive
    )
    fake_POST_resp = _generate_fake_OK_http_response(
        mocker, mock_POST_predictive_OK_return
    )

    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_predictive.requests.get",
        return_value=fake_GET_resp,
    )
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_predictive.requests.post",
        return_value=fake_POST_resp,
    )
    mocker.patch(
        "pseudonymization.pseudonimization_api.pseudonimize_predictive.uuid.uuid4",
        return_value=mock_new_uuid_value,
    )

    pat_pseudo = PseudonymizePredictive(PSEUDONYMIZATION_FILE)
    predictive_number = pat_pseudo.pseudonymize("2023-1112")
