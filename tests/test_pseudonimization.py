...
FAKE_RUN_FOLDER_FOR_COPY = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_run")
FAKE_RUN_FOR_TESTING = os.path.join(os.path.dirname(os.path.abspath(__file__)), "FAKE_RUN")

PSEUDONYMIZATION_FILES_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pseudonymization_files")
...
def _setup_pseudonymization_files():
    ...
    # create_file(PSEUDONYMIZATION_PATIENTS_FILE)
    # create_file(PSEUDONYMIZATION_SAMPLES_FILE)
    # create_file(PSEUDONYMIZATION_PREDICTIVE_FILE)
...
def test_get_all_predictive_numbers_pseudonymize_sample_sheet(mocker, generate_list_of_pseudo_pred_numbers):
    mocker.patch("pseudonymization.process.pseudonimizer.PseudonymizePredictive.pseudonimize", side_effect=generate_list_of_pseudo_pred_numbers)
    ...
    expected_pred_pseudo_numbers = [(pred, pseudo) for (pred, pseudo) in zip(expected_predictive_numbers, generate_list_of_pseudo_pred_numbers)]


    predictive_pseudo_numbers = RunPseudonimizer(FAKE_RUN_FOR_TESTING, PSEUDONYMIZATION_FILES_FOLDER)._get_all_predictive_numbers_pseudonymize_sample_sheet()
...
def test_rename_files_recursively():
    ...
    assert os.path.exists(os.path.join(FAKE_RUN_FOR_TESTING, "Analysis", "000-00_Output", "000-00", "000-00_Parameters.txt"))
...
def test_save_clinical_data(mocker, 
    ...
    mocker.patch("pseudonymization.pseudonimization_api.pseudonimize_sample.requests.get", side_effect=[fake_OK_sample_pseudo_response, fake_OK_sample_pseudo_response, fake_OK_patient_pseudo_response])
    mocker.patch("pseudonymization.pseudonimization_api.pseudonimize_sample.requests.get", side_effect=[fake_OK_sample_pseudo_response, fake_OK_sample_pseudo_response, fake_OK_patient_pseudo_response])
